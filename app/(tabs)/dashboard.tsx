...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function carregar() {
      try {
        ...
        setUsuario(u);
        const devs = resDevices.data ?? [];
        setNumDispositivos(String(devs.length));
      } catch (err) {
  ...
  const primeiroNome = usuario?.name?.split(" ")[0] ?? "Rafael";
  ...
  const planSub = () => {
    ...
    if (usuario.plan_expires_at) {
      return `Ativo até ${new Date(usuario.plan_expires_at).toLocaleDateString("pt-BR", { month: "short", year: "numeric" })}`;
    }
    return usuario.plan === "basico" ? "Gratuito" : "Ativo";
  };

  const metricas = [
    {
      label: "DISPOSITIVOS CONECTADOS",
      valor: numDispositivos,
      sub: "Todos online",
      subColor: "#16a34a",
    ...
    },
    {
      ...
      valor: usuario ? planLabel(usuario.plan) : "—",
      sub: planSub(),
      subColor: TEAL,
      icon: "ribbon-outline",
    },
    ...
    <View style={{ flex: 1, backgroundColor: "#f5f7f8" }}>
      <ScrollView style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.saudacao}>Bem-vindo, {primeiroNome}!</Text>
        ...
        >
          <View style={styles.dropdown}>
            <View style={styles.dropdownHeader}>
              ...
              <View>
                <Text style={styles.dropdownNome}>{usuario?.name ?? "Carregando..."}</Text>
                <Text style={styles.dropdownEmail}>{usuario?.email ?? ""}</Text>
              </View>
            </View>
